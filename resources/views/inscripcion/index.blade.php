...
@section('content')
    <section class="section">
        <div class="section-header">
            <h3 class="page__heading">Inscripcion</h3>
        </div>
                ...
                <div class="col-lg-12">
                    <div class="card">
                        <div class="card-header ">
                            <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#crear_nivel">
                                Nuevo
                            </button>
    ...
    @include('inscripcion.modal-create')
...
@section('scripts')
    <script>
       ...
       function selecMateria(){
        let obtenerid=document.getElementById('tipo_materia')
        let id=obtenerid.value;
        console.log(id);

        axios({
            method:'GET',
            url: "inscripcion/buscarmateria/" + id,
            data:{
                id:id,
            }
            ...
            var html_select = '<option value="" hidden>Seleccione Nivel</option>';
                    for (var i = 0; i < response.data.length; i++)
                        // console.log(response[i].nombre);
                         html_select += '<option value=" ' + response.data[i].id + ' ">' + response.data[i]
                        .nombre + '</option>';
