...
@if (count($actualizacion) > 0)
    ...
    <div class="datagrid">
        <table>
            ...
            <tbody>
                @foreach ($actualizacion as $actualizaciones)
                    <tr>
                        {{-- <th scope="row">{{$loop->iteration}}</th>s --}}
                        <td>{{ $actualizaciones->fecha_de_depreciacion }}</td>
                        <td>{{ $actualizaciones->nuevo_precio }}</td>
                        <td>{{ $actualizaciones->descripcion }}</td>
                        <td>{{ $actualizaciones->nombre_activo }}</td>
                        {{-- <td>
...
@else
    <div class="datagrid">
        ...
        <div class="alert alert-light  d-flex justify-content-center align-items-center" role="alert">
            ...
            <div class="alert-text">No se ha realizado una depreciacion para este activo.</div>
        </div>
    </div>
@endif
